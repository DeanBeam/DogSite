...
def weSharePuppies_view(request):
	...
	return render(request, "NewPuppies/listPairs.html", {"havePuppies": weHavePuppies, "waitPuppies": weWaitPuppies})

def weHadGrowth_view(request):
	weHadGrowth = Pair.objects.filter(pairIsPast=True)
	return render(request, "NewPuppies/growthPuppies.html", {"havePuppies": weHadGrowth})
...
def photoPuppies_detailview(request, pairLiter):
	...
	return render(request, "NewPuppies/photoLib.html", {"images":images, "pair":lookedPair})
